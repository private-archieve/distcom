...
const BlogDetail = () => {
  ...
  return (
    <>
      ...
      <div className="flex flex-col lg:flex-row justify-center items-start mt-5 lg:mt-20">
        <main className="w-full lg:max-w-4xl mx-auto p-4">
          ...
          <div>
            ...
            <span className="bg-slate-100 shadow-lg rounded-lg p-2 overflow-hidden">
              <Link href={`/Tags/${blogPost.Btags}`}>
                <FontAwesomeIcon icon={faTags} />
                {blogPost.Btags &&
                  tags.map(tag => {
                    const trimmedTag = tag;
                    return (
                      <Link key={trimmedTag} href={`/Tags/${trimmedTag}`} className="bg-gray-100 text-gray-700 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded">
                        {trimmedTag}
                      </Link>
                    );
                  })}
            ...
            <span className="float-right bg-slate-100 text-blue-700 text-xs font-semibold inline-block px-3 p-2 rounded-full shadow-lg">
              <Link href={`/Category/${blogPost.Bcategory}`} className="text-blue-500 hover:text-blue-700">
      ...
      {showSharePopup && <SharePopup url={`http://localhost:3000/Blogs/${blogPost.Bauthor.replace(' ', '')}/${blogPost.Burl}`} title={blogPost.Bname} onClose={handleClosePopup} />}
    </>
  );
};
