...
const CategoryButtons: React.FC<CategoryButtonsProps> = ({ categories, setSelectedCategory, selectedCategory }) => (
  <div className="flex justify-center items-center max-w-3xl mx-auto mt-20 my-8 bg-gray-100 shadow-lg rounded-lg p-4">
    <div className="flex space-x-2 overflow-x-auto">
      <button
        className={`text-sm px-4 py-2 rounded-md transition duration-300 ease-in-out transform hover:-translate-y-1 hover:shadow-md ${selectedCategory === null ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700'
          }`}
      ...
      {categories.map(category => (
        <button
          key={category}
          className={`text-sm px-4 py-2 rounded-md transition duration-300 ease-in-out transform hover:-translate-y-1 hover:shadow-md ${selectedCategory === category ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700'
            }`}
