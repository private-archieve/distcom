                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-6 gap-y-8">
                    {photos.map((photo, index) => (
                        <div key={index} className="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-shadow ease-in-out duration-300">
                            <Image src={photo.PhotoURL} alt="User Photo" className="w-full h-56 object-cover hover:scale-110 transition-transform duration-300 cursor-pointer" onClick={() => openModal(photo)} width={0}
                                height={0}
                                sizes="100vw"
                        ...
                        {selectedPhoto && (
                            <>
                                <div className="relative">
                                    <Image src={selectedPhoto.PhotoURL} alt="Selected" className="w-full h-auto max-h-96 object-contain p-4" width={0}
                                        height={0}
                                        sizes="100vw"
                                        style={{ width: '100%', height: 'auto' }} />
                                </div>
                                <div className="flex flex-col md:flex-row items-start p-6">
                                    <Image src={userData?.UsrProfileImage || siteData?.SiteDefaultProfileImageURL} alt={`${userData?.UsrDisplayName}'s profile`} className="h-20 w-20 rounded-full object-cover shadow-md mr-4 mb-4 md:mb-0" width={0}
                                        height={0}
                                        sizes="100vw"
                                        style={{ width: '100%', height: 'auto' }} />
                                    <div>
